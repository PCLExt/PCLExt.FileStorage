...
namespace PCLExt.FileStorage
{
    ...
    internal class NET4FolderImplementation : IFolder
    {
        ...
        {
            EnsureExists();
            return Directory.GetFiles(Path, searchPattern, (SearchOption) searchOption).Select(f => new NET4FileImplementation(f)).ToList<IFile>().AsReadOnly();
        }
        /// <inheritdoc />
        ...
        {
            EnsureExists();
            return Directory.GetDirectories(Path).Select(d => new NET4FolderImplementation(d, true)).ToList<IFolder>().AsReadOnly();
        }
        /// <inheritdoc />
